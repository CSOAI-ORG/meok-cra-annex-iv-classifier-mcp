...
def classify_product(
    ...
    if tier == "free":
        result["💡 UPGRADE"] = (
            f"You've used 1 of 3 free daily classifications. Upgrade to Pro (£79/mo) for "
            f"unlimited classifications + signed certs your Notified Body accepts: {STRIPE_199}"
        )
    result.update({
...
def audit_essential_requirements(
    ...
    if tier == "free":
        result["💡 UPGRADE"] = (
            ...
            f"unlimited audits + signed certs + remediation roadmap: {STRIPE_199}"
        )
    result.update({
        "product_name": product_name,
        "score_percent": score,
